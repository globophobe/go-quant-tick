...
func TestExchangesFromEnvAppliesConfiguredMarketThresholds(t *testing.T) {
	...
	if !threshold.Equal(quanttick.MustDecimal("10000")) {
		t.Fatalf("threshold = %s, want 10000", threshold)
	}
	threshold, ok = thresholds[quanttick.ExchangeSymbolKey(exchanges.BybitLinearName, "BTCUSDT")]
	if !ok {
	...
	threshold, ok = thresholds[quanttick.ExchangeSymbolKey(exchanges.BybitInverseName, "BTCUSD")]
	if !ok {
		t.Fatal("expected Bybit inverse BTCUSD threshold")
	}
	if !threshold.Equal(quanttick.MustDecimal("20000")) {
		t.Fatalf("threshold = %s, want 20000", threshold)
	}
